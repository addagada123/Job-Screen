...
function extractSkillsFromFile(filename) {
    ...
    // Add default skills based on keywords in filename
    ...
    // If no skills found, assign default based on likely category
    if (foundSkills.length === 0) {
        if (lowerName.indexOf('plumb') !== -1) {
            foundSkills = ['pipe fitting', 'drain cleaning', 'water heater'];
        } else if (lowerName.indexOf('electr') !== -1) {
            foundSkills = ['wiring', 'panel installation', 'circuit breaker'];
        } else if (lowerName.indexOf('mason') !== -1 || lowerName.indexOf('brick') !== -1) {
            foundSkills = ['brick laying', 'concrete work', 'tile installation'];
        } else if (lowerName.indexOf('carpent') !== -1) {
            foundSkills = ['furniture making', 'cabinet installation', 'door installation'];
        } else if (lowerName.indexOf('paint') !== -1) {
            foundSkills = ['interior painting', 'exterior painting', 'color matching'];
        } else if (lowerName.indexOf('driver') !== -1 || lowerName.indexOf('drive') !== -1) {
            foundSkills = ['defensive driving', 'cdl', 'route planning'];
        } else {
            // Default to general skills
            foundSkills = ['basic tools', 'safety procedures', 'work experience'];
        }
    }
...
function detectJobCategory(skills) {
    ...
    // If no clear match, try to detect from skills array
    if (maxScore === 0) {
        var skillsText = skills.join(' ').toLowerCase();
        if (skillsText.indexOf('pipe') !== -1 || skillsText.indexOf('drain') !== -1 || skillsText.indexOf('water') !== -1) {
            detectedCategory = 'plumbing';
        } else if (skillsText.indexOf('wire') !== -1 || skillsText.indexOf('circuit') !== -1 || skillsText.indexOf('voltage') !== -1) {
            detectedCategory = 'electrical';
        } else if (skillsText.indexOf('brick') !== -1 || skillsText.indexOf('cement') !== -1 || skillsText.indexOf('tile') !== -1) {
            detectedCategory = 'masonry';
        } else if (skillsText.indexOf('wood') !== -1 || skillsText.indexOf('furniture') !== -1 || skillsText.indexOf('cabinet') !== -1) {
            detectedCategory = 'carpentry';
        } else if (skillsText.indexOf('paint') !== -1 || skillsText.indexOf('color') !== -1) {
            detectedCategory = 'painting';
        } else if (skillsText.indexOf('drive') !== -1 || skillsText.indexOf('license') !== -1 || skillsText.indexOf('vehicle') !== -1) {
            detectedCategory = 'driving';
        }
    }
